...
async function cargarMazo() {
    ...
    try {
        ...
        const flashcards = await window.BabelAPI.listarFlashcards(gameState.mazo.id);
        ...
        // Crear sesion en el backend
        const userId = BabelUI.obtenerIdUsuario();
        if (userId) {
            try {
                const sesionData = await window.BabelAPI.crearSesion(userId, 'FLASHCARDS');
                if (sesionData && sesionData.IdSesion) {
                    gameState.idSesion = sesionData.IdSesion;
                }
            } catch (e) { console.warn('No se pudo crear sesion:', e); }
        }
...
// ═══════════════════════════════════════════════════════════════
async function persistirProgreso(acierto) {
    const userId = BabelUI.obtenerIdUsuario();
    const flashcard = gameState.flashcards[gameState.indiceActual];
    if (!userId || !flashcard) return;

    try {
        await window.BabelAPI.registrarProgresoFlashcard(userId, flashcard.id, acierto);

        const puntosGanados = acierto ? 10 : 2;
        await window.BabelAPI.registrarPuntos(userId, puntosGanados, 'FLASHCARDS');
    } catch (e) { console.warn('Error persistiendo progreso:', e); }
}
...
async function gameOver() {
    ...
    if (userId) {
        ...
        try {
            await window.BabelAPI.registrarGameOver({
                IdUsuario: userId,
                IdSesion: gameState.idSesion,
                CausaMuerte: 'Vidas agotadas en flashcards',
                ProgresoPerdido: gameState.dominadas,
                MensajeFinal: `Dominadas: ${gameState.dominadas}, Falladas: ${gameState.falladas}`
            });
            if (gameState.idSesion) {
                await window.BabelAPI.finalizarSesion(gameState.idSesion, {
                    EstadoSesion: 'GAME_OVER', VidasFinal: 0, PuntajeTotal: gameState.dominadas * 10, TiempoTotalSeg: tiempoSeg
                });
            }
            await BabelUI.evaluarLogros();
        } catch (e) { console.warn('Error sincronizando game over:', e); }
    }
}
...
// MISIÓN COMPLETADA
// ═══════════════════════════════════════════════════════════════
async function misionCompletada() {
    gameState.completado = true;
    ...
    if (userId) {
        const tiempoSeg = Math.floor((Date.now() - gameState.inicioTiempo) / 1000);
        const puntosVictoria = gameState.dominadas * 15 + gameState.vidas * 20;
        try {
            await window.BabelAPI.registrarPuntos(userId, puntosVictoria, 'FLASHCARDS');
            if (gameState.idSesion) {
                await window.BabelAPI.finalizarSesion(gameState.idSesion, {
                    EstadoSesion: 'COMPLETADA', VidasFinal: gameState.vidas, PuntajeTotal: puntosVictoria, TiempoTotalSeg: tiempoSeg
                });
            }
            await BabelUI.evaluarLogros();
        } catch (e) { console.warn('Error sincronizando victoria:', e); }
    }
}
